...
class RutorParser:
    ...
    def clean_display_title(self, full_title):
        ...
        t = re.split(
            r"SATRip|Web-DL|WEBRip|WEB-Rip|BDRip|BDRemux|HDTV|Rip|1080p|720p|4K|8K|HDR|SDR|UHD|HEVC|AVC|MVO|DUB|VO|\|",
            t,
            flags=re.IGNORECASE,
        )[0]
    ...
    def get_category_releases(self, category_id=1, page=0, max_retries=None):
        ...
        for row in rows:
            ...
            rutor_id = (
                rutor_id_match.group(1)
                if rutor_id_match
                else title_tag["href"].strip("/").split("/")[-1]
            )
    ...
    def search_releases(self, query, category_id=0, page=0, max_retries=None):
        import urllib.parse
