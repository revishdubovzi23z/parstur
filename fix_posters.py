...
def get_db_stats(cursor, video_cats):
    cursor.execute(
        f"SELECT COUNT(*) FROM items WHERE category_id IN {video_cats} AND (poster_url IS NULL OR poster_url = '')"
    )
    no_poster = cursor.fetchone()[0]
    cursor.execute(
        f"SELECT COUNT(*) FROM items WHERE category_id IN {video_cats} AND (kp_rating = 0 OR kp_rating IS NULL OR imdb_rating = 0 OR imdb_rating IS NULL)"
    )
        ...
        if not items:
            ...
            for idx, item_data in enumerate(items, 1):
                ...
                if data:
                    ...
                    cursor.execute(
                        "SELECT kp_rating, imdb_rating, poster_url, description FROM items WHERE id = ?",
                        (item_id,),
                    )
                    row_check = cursor.fetchone()
                    if (
                        row_check["kp_rating"] > 0
                        and row_check["imdb_rating"] > 0
                        and row_check["poster_url"]
                        and row_check["description"]
                    ):
                        cursor.execute(
                            "UPDATE items SET is_metadata_fixed = 1 WHERE id = ?",
                            (item_id,),
                        )
                ...
                if not client.is_limited:
                    cursor.execute(
                        f"UPDATE items SET {check_col} = 1 WHERE id = ?", (item_id,)
                    )
                    conn.commit()
