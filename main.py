...
running_processes = {"sync_video": None, "sync_other": None, "fix": None, "reprocess": None, "user": None, "cleanup": None, "rezka": None, "full_pipeline": None, "single_update": None, "active_pipeline_proc": None}
process_status = {"sync_video": "idle", "sync_other": "idle", "fix": "idle", "reprocess": "idle", "user": "idle", "cleanup": "idle", "rezka": "idle", "full_pipeline": "idle", "single_update": "idle"}
...
def check_any_running():
    ...
    for key, status in process_status.items():
        if status == "running":
            raise HTTPException(
                status_code=400, 
                detail=f"Другой процесс ({key}) уже запущен. Пожалуйста, дождитесь его завершения."
            )
...
def run_pipeline_task():
    ...
    steps = [
        ...
        ("fix_posters.py", ["poiskkino", "fix_poiskkino_log.txt"], "poiskkino", "fix_poiskkino_log.txt"),
    ...
    try:
        for script, args, key, log in steps:
            ...
            run_script_with_args(script, args, key, log, is_pipeline_step=True)
...
def run_script_with_args(script_name, args, status_key, log_file=None, is_pipeline_step=False):
    global process_status, running_processes
    process_status[status_key] = "running"
    ...
    try:
        if log_file:
            mode = "a" if is_pipeline_step else "w"
            f = open(log_file, mode, encoding="utf-8")
            if not is_pipeline_step:
                f.write(f"=== Запуск {script_name} ({datetime.now().strftime('%H:%M:%S')}) ===\n")
                f.flush()
        ...
        if proc.returncode == 0:
            status = "completed"
        elif proc.returncode == -15 or proc.returncode == 1 or proc.returncode == 15:
            status = "stopped"
        else:
            status = "error"
        ...
        try:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (status_key, start_time.strftime("%Y-%m-%d %H:%M:%S"), end_time.strftime("%Y-%m-%d %H:%M:%S"), duration, items_processed, total_items, status))
...
def stop_process(key: str):
    ...
    if key == "full_pipeline":
        ...
        # Пытаемся остановить текущий подпроцесс пайплайна
...
def start_fix(background_tasks: BackgroundTasks):
    ...
    background_tasks.add_task(
        run_script_with_args, "fix_posters.py", ["tech", log_file], "fix", log_file
    )
    return {"status": "started"}
...
def start_fix_uz(background_tasks: BackgroundTasks):
    ...
    log_file = "fix_uz_log.txt"
    with open(log_file, "w", encoding="utf-8") as f:
        f.write(f"=== Поиск (UZ API) {datetime.now().strftime('%H:%M:%S')} ===\n")
    background_tasks.add_task(
        run_script_with_args, "fix_posters.py", ["uz", log_file], "fix", log_file
    )
    return {"status": "started"}
...
def start_fix_poisk(background_tasks: BackgroundTasks):
    ...
    background_tasks.add_task(
        run_script_with_args, "fix_posters.py", ["poiskkino", log_file], "poiskkino", log_file
    )
    return {"status": "started"}
...
def reprocess_item(item_id: int, background_tasks: BackgroundTasks):
    # Очищаем лог перед запуском
    log_file = "single_update_log.txt"
    with open(log_file, "w", encoding="utf-8") as f:
        f.write(f"=== Обновление метаданных карточки ID {item_id} ({datetime.now().strftime('%H:%M:%S')}) ===\n")
    
    args = ["--force", "--id", str(item_id)]
    background_tasks.add_task(
        run_script_with_args, "reprocess_database.py", args, "single_update", log_file
    )
    return {"status": "started"}

@app.post("/api/update_item/{item_id}")
def update_item(item_id: int, background_tasks: BackgroundTasks):
    ...
    background_tasks.add_task(
        run_script_with_args, "single_item_update.py", [str(item_id)], "single_update", log_file
    )
    return {"status": "started"}
...
def get_watched_item_ids(cursor):
    cursor.execute("SELECT imdb_id, kp_id, title_norm, original_title_norm, item_year FROM user_ratings")
    ratings = cursor.fetchall()
    if not ratings: return set()
    rated_imdb_ids = set(); rated_kp_ids = set(); rated_names = {}
    for imdb_id, kp_id, title_norm, orig_norm, item_year in ratings:
        if imdb_id: rated_imdb_ids.add(imdb_id)
        if kp_id: rated_kp_ids.add(kp_id)
        for name in [title_norm, orig_norm]:
            if name:
                if name not in rated_names: rated_names[name] = []
                rated_names[name].append(item_year)
    watched_ids = set()
    if rated_imdb_ids:
        placeholders = ",".join("?" * len(rated_imdb_ids))
        cursor.execute(f"SELECT id FROM items WHERE imdb_id IN ({placeholders})", list(rated_imdb_ids))
        for row in cursor.fetchall(): watched_ids.add(row[0])
    if rated_kp_ids:
        placeholders = ",".join("?" * len(rated_kp_ids))
        cursor.execute(f"SELECT id FROM items WHERE kp_id IN ({placeholders})", list(rated_kp_ids))
        for row in cursor.fetchall(): watched_ids.add(row[0])
    if rated_names:
        ...
        for i in range(0, len(name_list), chunk_size):
            ...
            cursor.execute(f"SELECT sn.item_id FROM item_search_names sn WHERE sn.name_norm IN ({placeholders})", chunk)
            for row in cursor.fetchall(): watched_ids.add(row[0])
    return watched_ids
...
def get_categories(hide_rated: bool = False, hide_collected: bool = False):
    ...
    cursor.execute(f"SELECT c.id, c.name, (SELECT COUNT(*) FROM items i WHERE i.category_id = c.id AND i.is_ignored = 0 {not_in}) as count FROM categories c ORDER BY c.name")
    cats = [dict(row) for row in cursor.fetchall()]
    ids_str_cats = ",".join(map(str, VIDEO_CATEGORY_IDS))
    cursor.execute(f"SELECT COUNT(*) FROM items i WHERE category_id IN ({ids_str_cats}) AND is_ignored = 0 {not_in}")
    ...
    cursor.execute(f"SELECT COUNT(*) FROM items i WHERE i.category_id IN ({ids_str_cats}) AND i.is_ignored = 0 AND (i.poster_url IS NULL OR i.poster_url = '') {not_in}")
    no_poster_count = cursor.fetchone()[0]
    cursor.execute(f"SELECT COUNT(*) FROM items i WHERE i.category_id IN ({ids_str_cats}) AND i.is_ignored = 0 AND (i.kp_rating = 0 OR i.kp_rating IS NULL OR i.imdb_rating = 0 OR i.imdb_rating IS NULL) {not_in}")
    no_ratings_count = cursor.fetchone()[0]
    cursor.execute(f"SELECT COUNT(*) FROM items i WHERE i.category_id IN ({ids_str_cats}) AND i.is_ignored = 0 AND (i.kp_id IS NULL OR i.kp_id = '') {not_in}")
    no_kp_id_count = cursor.fetchone()[0]
    cursor.execute(f"SELECT COUNT(*) FROM items i WHERE i.category_id IN ({ids_str_cats}) AND i.is_ignored = 0 AND (i.imdb_id IS NULL OR i.imdb_id = '') {not_in}")
    no_imdb_id_count = cursor.fetchone()[0]
    cursor.execute(f"SELECT COUNT(*) FROM items i WHERE i.category_id IN ({ids_str_cats}) AND i.is_ignored = 0 AND (i.kp_id IS NULL OR i.kp_id = '') AND (i.imdb_id IS NULL OR i.imdb_id = '') {not_in}")
...
@app.get("/api/feed")
def get_feed(category_id: int = -1, collection_id: int = None, search: str = None, min_kp: float = 0.0, max_kp: float = 10.0, min_imdb: float = 0.0, max_imdb: float = 10.0, min_year: int = None, max_year: int = None, min_date: str = None, max_date: str = None, hide_ignored: bool = True, hide_rated: bool = False, hide_collected: bool = False, page: int = 1, limit: int = 40):
    conn = get_db(); cursor = conn.cursor()
    where_clauses = ["1=1"]; params = []
    if collection_id:
        where_clauses.append("items.id IN (SELECT item_id FROM collection_items WHERE collection_id = ?)")
        params.append(collection_id)
    elif category_id == -2:
        where_clauses.append("items.is_ignored = 1")
    else:
        ids_str = ",".join(map(str, VIDEO_CATEGORY_IDS))
        if category_id == -1: where_clauses.append(f"items.category_id IN ({ids_str})")
        elif category_id == -100: where_clauses.append(f"items.category_id IN ({ids_str}) AND (items.poster_url IS NULL OR items.poster_url = '')")
        elif category_id == -101: where_clauses.append(f"items.category_id IN ({ids_str}) AND (items.kp_rating = 0 OR items.kp_rating IS NULL OR items.imdb_rating = 0 OR items.imdb_rating IS NULL)")
        elif category_id == -102: where_clauses.append(f"items.category_id IN ({ids_str}) AND (items.kp_id IS NULL OR items.kp_id = '')")
        elif category_id == -103: where_clauses.append(f"items.category_id IN ({ids_str}) AND (items.imdb_id IS NULL OR items.imdb_id = '')")
        elif category_id == -104: where_clauses.append(f"items.category_id IN ({ids_str}) AND (items.kp_id IS NULL OR items.kp_id = '') AND (items.imdb_id IS NULL OR items.imdb_id = '')")
        elif category_id != 0: where_clauses.append("items.category_id = ?"); params.append(category_id)
        if hide_ignored: where_clauses.append("items.is_ignored = 0")
    if search:
        search_val = f"%{search.lower()}%"
        where_clauses.append(f"(items.title LIKE ? OR items.title_norm LIKE ? OR EXISTS (SELECT 1 FROM item_search_names sn WHERE sn.item_id = items.id AND sn.name_norm LIKE ?))")
        params.extend([f"%{search}%", search_val, search_val])
    if min_date: where_clauses.append("items.id IN (SELECT item_id FROM releases WHERE date_added >= ?)"); params.append(min_date)
    if max_date: where_clauses.append("items.id IN (SELECT item_id FROM releases WHERE date_added <= ?)"); params.append(max_date)
    if hide_rated:
        watched_ids = get_watched_item_ids(cursor)
        if watched_ids: where_clauses.append(f"items.id NOT IN ({','.join(map(str, watched_ids))})")
    if hide_collected and not collection_id: where_clauses.append("items.id NOT IN (SELECT item_id FROM collection_items)")
    if min_kp > 0: where_clauses.append("items.kp_rating >= ?"); params.append(min_kp)
    if max_kp < 10: where_clauses.append("items.kp_rating <= ?"); params.append(max_kp)
    if min_kp == 0 and (min_kp > 0 or max_kp < 10): where_clauses.append("items.kp_rating > 0")
    if min_imdb > 0: where_clauses.append("items.imdb_rating >= ?"); params.append(min_imdb)
    if max_imdb < 10: where_clauses.append("items.imdb_rating <= ?"); params.append(max_imdb)
    if min_imdb == 0 and (min_imdb > 0 or max_imdb < 10): where_clauses.append("items.imdb_rating > 0")
    if min_year: where_clauses.append("items.year >= ?"); params.append(min_year)
    if max_year: where_clauses.append("items.year <= ?"); params.append(max_year)
    where_sql = " AND ".join(where_clauses)
    cursor.execute(f"SELECT COUNT(DISTINCT items.id) FROM items WHERE {where_sql}", params)
    total_count = cursor.fetchone()[0]
    total_pages = (total_count + limit - 1) // limit if total_count > 0 else 1
    order_by = "items.ignored_at DESC, latest_release DESC" if category_id == -2 else ("ci.added_at DESC, latest_release DESC" if collection_id else "latest_release DESC NULLS LAST")
    ...
    for item in items:
        cursor.execute("SELECT * FROM releases WHERE item_id = ? ORDER BY date_added DESC", (item["id"],))
...
def ignore_item(item_id: int):
    ...
    cursor.execute("UPDATE items SET is_ignored = ?, ignored_at = ? WHERE id = ?", (new_state, ignored_at, item_id))
    conn.commit(); conn.close()
...
def reset_item(item_id: int, data: ResetFieldsRequest):
    ...
    if updates:
        ...
        if any(f in ["kp_id", "imdb_id", "poster", "ratings"] for f in data.fields):
            ...
            updates.append("checked_rezka = 0") # При смене ID нужно перепроверить Резку
...
def get_stats():
    ...
    cursor.execute(f"SELECT COUNT(*) FROM items WHERE category_id IN ({ids_str}) AND is_ignored = 0")
    total_video = cursor.fetchone()[0]
    
    cursor.execute(f"SELECT COUNT(*) FROM items WHERE category_id IN ({ids_str}) AND is_ignored = 0 AND (poster_url IS NULL OR poster_url = '')")
    no_poster = cursor.fetchone()[0]
    
    cursor.execute(f"SELECT COUNT(*) FROM items WHERE category_id IN ({ids_str}) AND is_ignored = 0 AND (kp_rating = 0 OR kp_rating IS NULL OR imdb_rating = 0 OR imdb_rating IS NULL)")
    no_ratings = cursor.fetchone()[0]
    
    cursor.execute(f"SELECT COUNT(*) FROM items WHERE category_id IN ({ids_str}) AND is_ignored = 0 AND (rezka_url IS NULL OR rezka_url = '')")
    no_rezka = cursor.fetchone()[0]
    
    cursor.execute(f"SELECT COUNT(*) FROM items WHERE category_id IN ({ids_str}) AND is_ignored = 0 AND (kp_id IS NULL OR kp_id = '') AND (imdb_id IS NULL OR imdb_id = '')")
    no_ids = cursor.fetchone()[0]
    
    # Последние запуски
    cursor.execute("SELECT job_type, MAX(end_time) as last_run FROM job_history GROUP BY job_type")
...
def get_job_history(limit: int = 20):
    conn = get_db(); cursor = conn.cursor()
    cursor.execute("SELECT * FROM job_history ORDER BY start_time DESC LIMIT ?", (limit,))
...
def get_collections():
    conn = get_db(); cursor = conn.cursor()
    cursor.execute("SELECT c.*, COUNT(ci.item_id) as count FROM collections c LEFT JOIN collection_items ci ON c.id = ci.collection_id GROUP BY c.id ORDER BY c.sort_order ASC, c.name ASC")
    collections = [dict(row) for row in cursor.fetchall()]
    conn.close(); return collections
...
def create_collection(data: CollectionCreate):
    ...
    try:
        ...
        conn.commit()
    except sqlite3.IntegrityError: return {"status": "error", "message": "Коллекция существует"}
    finally: conn.close()
    return {"status": "success"}
...
class CollectionItemRequest(BaseModel): item_id: int
@app.post("/api/collections/{collection_id}/toggle")
def toggle_collection_item(collection_id: int, data: CollectionItemRequest):
    conn = get_db(); cursor = conn.cursor()
    cursor.execute("SELECT 1 FROM collection_items WHERE collection_id = ? AND item_id = ?", (collection_id, data.item_id))
    if cursor.fetchone():
        cursor.execute("DELETE FROM collection_items WHERE collection_id = ? AND item_id = ?", (collection_id, data.item_id))
        action = "removed"
    else:
        cursor.execute("INSERT INTO collection_items (collection_id, item_id, added_at) VALUES (?, ?, ?)", (collection_id, data.item_id, datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
        action = "added"
    conn.commit(); conn.close(); return {"status": "success", "action": action}

@app.get("/api/item_collections/{item_id}")
def get_item_collections(item_id: int):
    conn = get_db(); cursor = conn.cursor()
    cursor.execute("SELECT collection_id FROM collection_items WHERE item_id = ?", (item_id,))
    ids = [row[0] for row in cursor.fetchall()]
    conn.close(); return ids

class SaveOrderRequest(BaseModel): order: list[int]
@app.post("/api/collections/save_order")
def save_collections_order(data: SaveOrderRequest):
    conn = get_db(); cursor = conn.cursor()
    for i, col_id in enumerate(data.order): cursor.execute("UPDATE collections SET sort_order = ? WHERE id = ?", (i, col_id))
    conn.commit(); conn.close(); return {"status": "success"}
...
def get_sync_log(log_type: str = "video"):
    ...
    if not filename:
         return JSONResponse(content={"log": "Неизвестный тип лога", "filename": ""}, status_code=400)
...
def clear_log(log_type: str = "video"):
    ...
    filename = log_files.get(log_type)
    if not filename: return {"status": "error", "message": "Unknown log type"}
...
def sync_user_data(background_tasks: BackgroundTasks):
    with open("user_sync_log.txt", "w", encoding="utf-8") as f: f.write("=== Старт ===\n")
