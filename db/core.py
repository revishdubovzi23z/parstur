...
class DbCore:
    ...
    def _apply_migrations(self) -> None:
        ...
        the target user_version. Each file is executed statement by
        statement; on failure the database is left at the previous
        user_version. 'duplicate column name' errors are gracefully
        ignored to support idempotent migrations on existing databases.
        """
        import os as _os
        import re as _re
        ...
        with self._conn() as c:
            ...
            for fname in files:
                ...
                for line in script.splitlines():
                    current_stmt += line + "\n"
                    if sqlite3.complete_statement(current_stmt):
                        try:
                            c.execute(current_stmt)
                        except sqlite3.OperationalError as e:
                            if "duplicate column name" in str(e).lower():
                                logger.info(f"[DB] Migration {fname} ignored duplicate column: {e}")
                            else:
                                raise
                        current_stmt = ""
